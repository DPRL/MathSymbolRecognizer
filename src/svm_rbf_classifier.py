...
def main():
    #usage check...
    if len(sys.argv) < 5:
        print("Usage: python svm_rbf_classifier.py training testing C Gamma eval [probab]")
        print("Where")
        print("\ttraining\t= Path to training set")
        print("\ttesting\t\t= Path to testing set")
        print("\tC\t\t= C parameter of the RBF SVM")
    ...
    try:
        rbf_C = float(sys.argv[3])
    except:
        print("Invalid value for C")
        return

    try:
        rbf_Gamma = float(sys.argv[4])
    except:
        print("Invalid value for Gamma")
        return

    if len(sys.argv) >= 6:
        try:
            evaluate = int(sys.argv[5]) > 0
        except:
    ...
    if len(sys.argv) >= 7:
        try:
            make_probabilistic = int(sys.argv[6]) > 0
        except:
    ...
    print("Current Gamma = " + str(rbf_Gamma))
    ...
    print("...training...")
    classifier = svm.SVC(C=rbf_C, gamma=rbf_Gamma, probability=make_probabilistic)
